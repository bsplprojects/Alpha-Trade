...
const PayoutPerformanceBonus = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const { performanceBonusMutation } = usePayout();
  ...
  const totalAmount = useMemo(() => {
    return performanceBonusMutation.data?.data?.data?.reduce(
      (total, d) => total + parseFloat(d.amount),
      0
    );
  }, [performanceBonusMutation.data?.data]);

  const filteredMembers = useMemo(() => {
    const members = performanceBonusMutation.data?.data?.data || [];
  ...
  }, [performanceBonusMutation.data?.data, fromDate, toDate]);
  ...
  return (
    <>
      ...
      {performanceBonusMutation.data?.data?.status === "SUCCESS" && (
...
export default PayoutPerformanceBonus;
