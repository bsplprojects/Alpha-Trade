  ...
  return (
    <div className="space-y-4 px-3">
      {data.map((item) => (
        <div
          key={item.ID}
          className="p-4 rounded-2xl border bg-white shadow-sm hover:shadow-md transition-all"
        >
          {/* Top Row */}
          ...
          {item.AdminReply && (
            <div className="mt-3 p-3 rounded-xl bg-orange-50 border border-orange-200">
              <p className="text-xs text-orange-600 mb-1">Reply</p>
          ...
          <div className="flex items-center justify-between mt-3 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              <Clock size={12} />
              <span>{item.RequestDate?.split(" ")[0]}</span>
            </div>
